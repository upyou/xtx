<script setup>
import { ref } from "vue"
import {getCheckout,submitCheckout} from '@/api/checkout'
...
const submit = async ()=>{
 const res  = await submitCheckout(
  ...
  const payId = res.data.result.id
  router.push({path: '/pay', query: {id: payId}})
  useCateStore().newCateList()
}
getCheckoutInfo()
</script>
...
<template>
  ...
  <div class="addressWrapper">
    <div class="text item" 
    @click="addressData(item)" 
    v-for="item in checkInfo.userAddresses"  :key="item.id"
    :class="{active: item.id === addr.id}" >
      <ul>
  ...
  <template #footer>
    <span class="dialog-footer">
      ...
      <el-button type="primary" @click="updataAddress">确定</el-button>
    </span>
  </template>
</el-dialog>
  <!-- 添加地址 -->
